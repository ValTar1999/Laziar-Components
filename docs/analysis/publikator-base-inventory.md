# Инвентаризация base — publikator

> Источник: `c:\Users\user\Documents\Work-2\publikator\src\app\common\components\base`
...
| `avatar/avatar.ts` | AvatarSize, AvatarSizeKey, AVATAR_SIZES, Colors, TColors, buildAvatarInitials | model/types | Экспорты: AvatarSize, AvatarSizeKey, AVATAR_SIZES, Colors, TColors, buildAvatarInitials |
...
| `buttonGroup/button-group.component.css` | button-group.component.css | styles | Стили компонента |
...
| `messageChat/message-chat.component.html` | message-chat.component.html | template | HTML-шаблон компонента |
| `messageChat/message-chat.component.ts` | MessageChatComponent | component | Пузырь сообщения чата (`app-message-chat`) |
...
| `select/select.component.ts` | SelectComponent | component | Выпадающий select (`app-select`); ControlValueAccessor |
...
| `table/table.component.html` | table.component.html | template | HTML-шаблон компонента |
| `table/table.component.ts` | TableComponent | component | Таблица данных с пагинацией и кастомными ячейками (`app-table`) |
...
| `textarea/textarea.component.ts` | TextareaComponent | component | Многострочное текстовое поле (`app-textarea`); ControlValueAccessor |
| `toastNotification/toast-container.component.ts` | ToastContainerComponent | component | Toast-уведомления и контейнер/сервис (`app-toast-container`) |
| `toastNotification/toast-notification.component.html` | toast-notification.component.html | template | HTML-шаблон компонента |
| `toastNotification/toast-notification.component.ts` | ToastNotificationComponent | component | Toast-уведомления и контейнер/сервис (`app-toast-notification`) |
| `toastNotification/toast.service.ts` | ToastService | service | Toast-уведомления и контейнер/сервис |
...
- **AvatarComponent** (`app-avatar`) — inputs: 11, outputs: 0, CVA: нет, lifecycle: —
...
- **ButtonComponent** (`app-button`) — inputs: 14, outputs: 1, CVA: нет, lifecycle: —
- **ButtonGroupComponent** (`app-button-group`) — inputs: 1, outputs: 0, CVA: нет, lifecycle: —
- **CardComponent** (`app-card`) — inputs: 5, outputs: 0, CVA: нет, lifecycle: ngOnInit
...
- **InputFieldComponent** (`app-input-field`) — inputs: 11, outputs: 1, CVA: да, lifecycle: ngOnInit
...
- **MessageChatComponent** (`app-message-chat`) — inputs: 4, outputs: 0, CVA: нет, lifecycle: ngOnChanges, ngAfterViewInit
- **ProgressCircleComponent** (`app-progress-circle`) — inputs: 3, outputs: 0, CVA: нет, lifecycle: —
- **SelectComponent** (`app-select`) — inputs: 6, outputs: 1, CVA: да, lifecycle: —
...
- **TableComponent** (`app-table`) — inputs: 25, outputs: 3, CVA: нет, lifecycle: ngOnDestroy, ngAfterViewInit
- **TabsComponent** (`app-tabs`) — inputs: 7, outputs: 1, CVA: нет, lifecycle: —
- **TextareaComponent** (`app-textarea`) — inputs: 6, outputs: 0, CVA: да, lifecycle: —
- **ToastContainerComponent** (`app-toast-container`) — inputs: 0, outputs: 0, CVA: нет, lifecycle: ngOnInit, ngOnDestroy
- **ToastNotificationComponent** (`app-toast-notification`) — inputs: 3, outputs: 1, CVA: нет, lifecycle: —
...
- **VideoPlayerComponent** (`app-video-player`) — inputs: 1, outputs: 0, CVA: нет, lifecycle: ngOnDestroy, ngAfterViewInit

## Не-компоненты

- **Сервис** `toastNotification/toast.service.ts` — ToastService: Toast-уведомления и контейнер/сервис
- **model** `alert/alert.ts` — экспорты: Variants, TVariants, Colors, TColors, AlertStyles, AlertStylesMap
- **model** `avatar/avatar.ts` — экспорты: AvatarSize, AvatarSizeKey, AVATAR_SIZES, Colors, TColors, buildAvatarInitials
